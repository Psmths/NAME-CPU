...
dr = ["ADD","SUB","LSL","RSR","AND","ORR","EOR","XCH","MOV","LDR","STR","LDB","STB","CMP"]
ir = ["XXX", "ADD","SUB","LSL","RSR","AND","ORR","EOR","MOV","CMP"]
r = ["RA","RB","RC","RD","PC","FP","BP","RX","RF","RL",]

for line in source:
	
	offset = re.search('^NOP*', line)
	if (offset):
		image.append(0x00)
		image.append(0x00)
		
	offset = re.search('^RTI*', line)
	if (offset):
		image.append(0x00)
		image.append(0x04)
	
	offset = re.search('^HLT*', line)
	if (offset):
		image.append(0x00)
		image.append(0x02)
		
	offset = re.search('^RET*', line)
	if (offset):
		image.append(0x00)
		image.append(0x03)
	...
	for m in dr:
		...
		if (s_found):
			...
			try:
				...
				#This is an immediate register commands
